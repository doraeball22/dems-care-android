...
public class VideoItemDao implements Parcelable {
    // ใช้ เว็บ jsonschema2pojo.org for generate ได้ json -> gson

    public static final Creator<VideoItemDao> CREATOR = new Creator<VideoItemDao>() {
        @Override
        public VideoItemDao createFromParcel(Parcel in) {
            return new VideoItemDao(in);
        }

        @Override
        public VideoItemDao[] newArray(int size) {
            return new VideoItemDao[size];
        }
    };
    ...
    protected VideoItemDao(Parcel in) {
        id = in.readString();
        title = in.readString();
